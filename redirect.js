...
addEventListener('fetch', async event => {
  ...
  if (url.match('/redirect[0-3]')) event.respondWith(bulkRedirects(request, REDIRECT_MAP))
  if (url.endsWith('/follow')) event.respondWith(followRedirects(someURLThatSendsRedirects))
  if (url.includes('/send')) event.respondWith(redirectResponse(someURLToRedirectTo))
  if (url.includes('/device'))
    event.respondWith(redirectOnDeviceType(event.request, 'mobile', MOBILE_REDIRECT_HOSTNAMES))
