...
addEventListener('fetch', async event => {
  ...
  if (url.endsWith('/follow')) return event.respondWith(followRedirects(someURLThatSendsRedirects))
  if (url.includes('/send')) return event.respondWith(redirectResponse(someURLToRedirectTo))
  if (url.includes('/device'))
    return event.respondWith(
      redirectOnDeviceType(event.request, 'mobile', MOBILE_REDIRECT_HOSTNAMES)
    )
